...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (dotProduct(ray.direction, normal) > 0)
        return inter;
    double u, v, t_tmp = 0;
    Vector3f pvec = crossProduct(ray.direction, e2); //pvec is equal to  S1
    double det = dotProduct(e1, pvec); //det's abs is equal to dotProduct(S1,E1);
    if (fabs(det) < EPSILON)
        return inter;

    double det_inv = 1. / det;
    Vector3f tvec = ray.origin - v0; //tvec is euqal to S
    u = dotProduct(tvec, pvec) * det_inv; //equal to b1
    if (u < 0 || u > 1)
        return inter;
    Vector3f qvec = crossProduct(tvec, e1); //equal to S2
    v = dotProduct(ray.direction, qvec) * det_inv; //equal to b2
    if (v < 0 || u + v > 1)
        return inter;
    t_tmp = dotProduct(e2, qvec) * det_inv; //equal to t
    if(t_tmp<=0)
        return inter;
